...
function _init_area(province, city, county = null) { //初始化函数
	areaSelects.push("#" + province);
	areaSelects.push("#" + city);
	if (county) {
		areaSelects.push("#" + county);
	}
	for (i = 0; i < areaSelects.length - 1; i++) {
		$(areaSelects[i]).onchange = new Function("change(" + (i + 1) + ")");
	}
	change(0);
}